...
# --- NEW FUNCTION TO DOWNLOAD FILE BYTES ---
def download_file_bytes(file_path: str):
    """Downloads the raw bytes of a file from Supabase Storage."""
    supabase = init_connection()
    try:
        file_bytes = supabase.storage.from_("receipts").download(path=file_path)
        return file_bytes
    except Exception as e:
        st.error(f"Could not download file '{file_path}': {e}")
        return None
...
def generate_report_xml(report_data: pd.Series, expenses_data: pd.DataFrame, submitter_name: str):
    ...
    create_sub_element(default_values_hdr, "Member", "").set("Name", "ShipTo")
    ...
    create_sub_element(row_header, "TaxHoldbackAmnt", "0"); create_sub_element(row_header, "ShipAmnt", "0")
    create_sub_element(row_header, "SurchargeAmnt", "0"); create_sub_element(row_header, "ShipVia", "")
    create_sub_element(row_header, "ShipTax", "0"); create_sub_element(row_header, "TotHoldBackAmnt", "0")
    create_sub_element(row_header, "PurchHoldbackAmnt", "0"); create_sub_element(row_header, "DetHoldBackAmnt", "0")
    create_sub_element(row_header, "Balance", f"{report_data.get('total_amount', 0):.4f}")
    create_sub_element(row_header, "Buyer", ""); create_sub_element(row_header, "ExTemplId", "Purchase Order", {"MemberName": "TmplName"})
