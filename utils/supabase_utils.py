...
def get_expenses_for_report(report_id):
    ...
    supabase = init_connection()
    try:
        # FIX: The select statement now also retrieves gl_account from the joined table.
...
def generate_report_xml(report_data: pd.Series, expenses_data: pd.DataFrame, submitter_name: str):
    ...
    # Add other placeholder tags to match structure
    ...
    create_sub_element(row_header, "TotHoldBackAmnt", "0")
    create_sub_element(row_header, "PurchHoldbackAmnt", "0")
    create_sub_element(row_header, "DetHoldBackAmnt", "0")
    ...
    create_sub_element(row_header, "ExTemplId", "Purchase Order", {"MemberName": "TmplName"})
