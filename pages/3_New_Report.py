...
if uploaded_receipt:
    with st.spinner("Processing OCR and uploading receipt..."):
        ...
        try:
            upload_resp = supabase.storage.from_("receipts").upload(unique_filename, file_bytes)
            # Handle both v1 and v2 responses
            if isinstance(upload_resp, dict):
                err = upload_resp.get("error")
            else:
                err = getattr(upload_resp, "error", None)
            if err:
    ...
    parsed_data = {
        "date": None, "vendor": "", "total_amount": 0.0,
...
if st.session_state.current_report_items:
    ...
    st.dataframe(
        pd.DataFrame(st.session_state.current_report_items)[
