...
from utils import supabase_utils as su # --- FIX: Import the supabase_utils module ---
...
if uploaded_receipt:
    with st.spinner("Processing OCR and uploading receipt..."):
        ...
        if line_items_from_ocr:
            ...
            st.session_state.edited_line_items = df
        else:
            st.session_state.edited_line_items = pd.DataFrame()
else:
...
# --- Line Item Display ---
if not st.session_state.get('edited_line_items', pd.DataFrame()).empty:
    st.markdown("---")
    ...
    st.session_state.edited_line_items = edited_df
...
with st.form("expense_item_form"):
    ...
    if submitted_item:
        if vendor and amount > 0 and overall_category:
            processed_line_items = st.session_state.get('edited_line_items', pd.DataFrame()).to_dict('records')
            ...
            with st.spinner("Submitting report..."):
                ...
                if report_id:
                    ...
                    if all_items_saved:
                        ...
                        st.error("Critical Error: The report header was saved, but one or more expense items failed to save. Your items have NOT been cleared. Please review any errors above and try submitting again.")
                else:
                    st.error("Critical Error: Failed to create the main report entry in the database.")
